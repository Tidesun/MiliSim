...
def generate_exon_indicator_for_isoform_single_gene(args):
    gene_name,chr_name,single_gene_raw_isoform_exons_dict,exon_list,single_gene_gene_points_dict,READ_LEN,READ_JUNC_MIN_MAP_LEN = args
...
def generate_exon_indicator_for_isoform_single_gene_helper(worker_id):
    with open(config.output_path+f'/temp/annotation/{worker_id}','rb') as f:
        list_of_args = pickle.load(f)
    all_res = ({},{},{})
    for args in list_of_args:
        gene_name,chr_name = args[0],args[1]
        new_res = generate_exon_indicator_for_isoform_single_gene(args)
        for i in range(3):
            if chr_name not in all_res[i]:
                all_res[i][chr_name] = {}
            all_res[i][chr_name][gene_name] = new_res[i]
    return all_res
def generate_exon_indicator_for_isoform(gene_exons_dict,gene_points_dict,raw_isoform_exons_dict,threads,READ_LEN,READ_JUNC_MIN_MAP_LEN):
    ...
    if threads == 1:
        ...
        if extra:
            chunksize += 1
        futures = []
        Path(config.output_path+f'/temp/annotation/').mkdir(exist_ok=True,parents=True)
        for i in range(threads):
            start = i*chunksize
            if i == threads:
                end = len(list_of_all_genes_chrs)
            else:
                end = (i+1)*chunksize
            pickled = []
            for gene_name,chr_name in list_of_all_genes_chrs[start:end]:
                pickled += [(gene_name,chr_name,raw_isoform_exons_dict[chr_name][gene_name],gene_exons_dict[chr_name][gene_name],gene_points_dict[chr_name][gene_name],READ_LEN,READ_JUNC_MIN_MAP_LEN)]
            with open(config.output_path+f'/temp/annotation/{i}','wb') as f:
                pickle.dump(pickled,f)
            futures.append(pool.apply_async(generate_exon_indicator_for_isoform_single_gene_helper,(i,)))
        print('Using {} threads'.format(threads),flush=True)
        for future in futures:
            new_isoforms_regions_len_dict,new_gene_regions_dict,new_genes_regions_len_dict = future.get()
            for chr_name in new_isoforms_regions_len_dict:
                isoforms_regions_len_dict[chr_name].update(new_isoforms_regions_len_dict[chr_name])
                gene_regions_dict[chr_name].update(new_gene_regions_dict[chr_name])
                genes_regions_len_dict[chr_name].update(new_genes_regions_len_dict[chr_name])
        pool.close()
        pool.join()
