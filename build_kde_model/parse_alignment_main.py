...
def parse_alignment_iteration(alignment_file_path,READ_LEN, READ_JUNC_MIN_MAP_LEN,map_f,temp_queue,long_read,aln_line_marker):
    ...
    with open(alignment_file_path, 'r') as aln_file:
        ...
        local_gene_regions_read_mapping = {}
        ...
        for line in aln_file:
                ...
                if (mapping['read_mapped']):
                    for mapping_area in [random.choice(mapping['mapping_area'])]:
                        ...
                        if rname not in local_gene_regions_read_count:
                            local_gene_regions_read_count[rname],local_gene_regions_read_length[rname],local_gene_regions_read_mapping[rname] = {},{},{}
                        if gname not in local_gene_regions_read_count[rname]:
                            local_gene_regions_read_count[rname][gname],local_gene_regions_read_length[rname][gname],local_gene_regions_read_mapping[rname][gname] = {},{},{}
                        if region_name not in local_gene_regions_read_count[rname][gname]:
                            local_gene_regions_read_count[rname][gname][region_name],local_gene_regions_read_length[rname][gname][region_name],local_gene_regions_read_mapping[rname][gname][region_name] = 0,[],[]
                        ...
                        if long_read:
                            ...
                            local_gene_regions_read_mapping[rname][gname][region_name].append(mapping)
                    buffer_size += 1
            ...
            if buffer_size > max_buffer_size:
                temp_queue.put((local_gene_regions_read_count,local_gene_regions_read_length,local_gene_regions_read_mapping))
                local_gene_regions_read_count,local_gene_regions_read_length,local_gene_regions_read_mapping = {},{},{}
                buffer_size = 0
        if buffer_size > 0:
            temp_queue.put((local_gene_regions_read_count,local_gene_regions_read_length,local_gene_regions_read_mapping))
    return 
def mapping_listener(temp_queue,gene_regions_read_count,gene_regions_read_length,gene_regions_read_mapping):
    num_mapped_lines = 0
    num_lines = 0
    while True:
        msg = temp_queue.get()
        if msg == 'kill':
            ...
            local_gene_regions_read_count,local_gene_regions_read_length,local_gene_regions_read_mapping = msg
            for rname in local_gene_regions_read_count:
                for gname in local_gene_regions_read_count[rname]:
                    for region in local_gene_regions_read_count[rname][gname]:
                        ...
                        gene_regions_read_mapping[rname][gname][region] += local_gene_regions_read_mapping[rname][gname][region]
    ...
    return gene_regions_read_count,gene_regions_read_length,gene_regions_read_mapping,num_mapped_lines
...
def parse_alignment(alignment_file_path,READ_LEN,READ_JUNC_MIN_MAP_LEN,gene_points_dict,gene_range,gene_interval_tree_dict,gene_regions_dict,genes_regions_len_dict,gene_isoforms_length_dict,long_read,filtering,threads):
    ...
    for rname in gene_regions_dict:
        gene_regions_read_count[rname],gene_regions_read_length[rname],gene_regions_read_mapping[rname] = {},{},{}
        for gname in gene_regions_dict[rname]:
            gene_regions_read_count[rname][gname],gene_regions_read_length[rname][gname],gene_regions_read_mapping[rname][gname] = {},{},{}
            # if (not long_read):
            ...
            for region in per_gene_regions_dict:
                gene_regions_read_mapping[rname][gname][region] = []
    ...
    # with open('aln.pkl','wb') as f:
    #     pickle.dump([points_dict,interval_tree_dict,filtered_gene_regions_dict,
    #                 start_pos_list, start_gname_list, end_pos_list, end_gname_list,
    #                 READ_LEN, READ_JUNC_MIN_MAP_LEN, CHR_LIST],f)
    ...
    watcher = pool.apply_async(mapping_listener, args=(temp_queue,gene_regions_read_count,gene_regions_read_length,gene_regions_read_mapping))
    ...
    for future in futures:
        future.get()
    temp_queue.put('kill')
    gene_regions_read_count,gene_regions_read_length,gene_regions_read_mapping,num_mapped_lines = watcher.get()
    pool.close()
    pool.join()
    read_lens = []
    ...
    if (long_read):
        ...
        return gene_regions_read_count,gene_regions_read_length,gene_regions_read_mapping,sum(read_lens),num_long_reads,filtered_gene_regions_read_length
    else:
